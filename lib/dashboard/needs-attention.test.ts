...
describe("buildCaregiverFocusToday", () => {
  ...
  it("keeps tasks completed today and sorts them after open work", () => {
    const items = buildCaregiverFocusToday({
      tasks: [
        task({
          id: "open",
    ...
    assert.ok(items.some((i) => i.id === "task-finished" && i.reason === "done"));
    assert.ok(!items.some((i) => i.id === "task-old-done"));
    const openIdx = items.findIndex((i) => i.id === "task-open");
    const doneIdx = items.findIndex((i) => i.id === "task-finished");
    assert.ok(openIdx >= 0 && doneIdx >= 0 && openIdx < doneIdx);
  });

  it("shows open recurring due tomorrow when today was never completed", () => {
    const items = buildCaregiverFocusToday({
      tasks: [
        task({
          id: "next-only",
          title: "Morning meds",
          dueAt: "2026-08-11T14:00:00.000Z", // tomorrow
    ...
    assert.equal(items.length, 1);
    assert.equal(items[0]?.title, "Morning meds");
  });

  it("hides open recurring due tomorrow when today was already completed", () => {
    const items = buildCaregiverFocusToday({
      tasks: [
        task({
          id: "done-today",
    ...
    assert.ok(items.some((i) => i.id === "task-done-today" && i.reason === "done"));
    assert.ok(!items.some((i) => i.id === "task-next"));
  });
});
